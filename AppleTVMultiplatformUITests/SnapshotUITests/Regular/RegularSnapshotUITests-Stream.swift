...
final class RegularSnapshotUITests_Stream: XCTestCase {
    ...
    private func iPad(app: XCUIApplication, playlist: Playlist) async throws {
        ...
        snapshotUtils.assertSnapshot(named: env.snapshotName(context: "stream"), app: app, localized: false, precision: 0.9998)
        ...
        try await app.navigationBars["Comedy"].buttons["settings"].firstMatch.makeTap(wait: .seconds(1))
        XCTAssertEqual(app.buttons["cancel"].firstMatch.isEnabled, true)
        XCTAssertEqual(app.buttons["confirm"].firstMatch.isEnabled, false)
        XCTAssertEqual(app.switches["pip-toggle"].firstMatch.value as? String, "1")
        try await app.switches["pip-toggle"].firstMatch.switches.firstMatch.makeTap(wait: .seconds(0))
        XCTAssertEqual(app.switches["pip-toggle"].firstMatch.value as? String, "0")
        XCTAssertEqual(app.buttons["cancel"].firstMatch.isEnabled, false)
        XCTAssertEqual(app.buttons["confirm"].firstMatch.isEnabled, true)
        try await app.buttons["confirm"].firstMatch.makeTap(wait: .seconds(1))
        XCTAssertEqual(app.scrollViews["details"].firstMatch.staticTexts.element(boundBy: 0).label, "15:00 - 17:00: 21 Jump Street")
        try await app.navigationBars["Comedy"].buttons["settings"].firstMatch.makeTap(wait: .seconds(1))
        XCTAssertEqual(app.switches["pip-toggle"].firstMatch.value as? String, "0")
    ...
    private func iPhone(app: XCUIApplication, playlist: Playlist) async throws {
        ...
        snapshotUtils.assertSnapshot(named: env.snapshotName(context: "stream"), app: app, localized: false, precision: 0.9999)
        XCTAssertEqual(app.scrollViews["details"].firstMatch.staticTexts.element(boundBy: 0).label, "15:00 - 17:00: 21 Jump Street")
        XCTAssertEqual(app.scrollViews["details"].firstMatch.staticTexts.element(boundBy: 1).label, "17:00 - 19:00: Palm Springs")
        XCTAssertEqual(app.scrollViews["details"].firstMatch.staticTexts.element(boundBy: 2).label, "19:00 - 21:00: Vacation")
    ...
    private func macOS(app: XCUIApplication, playlist: Playlist) async throws {
        ...
        try await snapshotUtils.assertSnapshot(named: env.snapshotName(context: "stream"), app: app, localized: false, precision: 0.998)
